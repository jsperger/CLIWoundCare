...
Load the data
```{r, message = FALSE}
grid <- readxl::read_xlsx("/Users/nikkifreeman/Dropbox/Consulting project/Data/grid2NoMRN.xlsx")
mort <- readxl::read_xlsx("/Users/nikkifreeman/Dropbox/Consulting project/Data/mortNoMRN.xlsx")
proc <- readxl::read_xlsx("/Users/nikkifreeman/Dropbox/Consulting project/Data/procGroupedNoMRN.xlsx")
procLat <- readxl::read_xlsx("/Users/nikkifreeman/Dropbox/Consulting project/Data/procLatNoMRN.xlsx")
ptFile <- readxl::read_xlsx("/Users/nikkifreeman/Dropbox/Consulting project/Data/masterPtFileNoMRN.xlsx")
```
...
Created ad hoc for the manuscript (per e-mail from Kate McGinigle on 3 February 2021)

```{r}
mort1yr <- left_join(primaryProc, mutate(mort, 
                              ptNum = as.character(ptNum), 
                              ptSide = paste0(ptNum, hemodynamicLaterality)), 
          by = c("ptNum", "ptSide")) %>%
  select(ptSide, ptNum, deathDate, timeToDeathFromStudyStart) %>%
  mutate(mort1yr = if_else(!is.na(timeToDeathFromStudyStart) & timeToDeathFromStudyStart <= 365, 1, 0))
```

## Create the amputation outcome

```{r}
amp2yr_temp <- procLat %>% mutate(ptSide = paste0(ptNum, laterality)) %>% 
...
amp2yr <- amp2yr_temp %>% filter(timeToAmputation <= years(2)) %>%
  select(ptNum, ptSide) %>%
...
Ad hoc request for the manuscript (e-mail from Kate McGinigle on 3 February 2021 to Nikki and John)

```{r}
amp1yr <- amp2yr_temp %>% filter(timeToAmputation <= years(1)) %>%
  select(ptNum, ptSide) %>%
  mutate(majorAmp1yr = 1) %>%
  distinct()
```
...
preAnalyticData <- left_join(primaryProc, mort2yr, by = c("ptNum", "ptSide")) %>%
  left_join(select(mort1yr, ptSide, ptNum, mort1yr), by = c("ptNum", "ptSide")) %>% # Added 11 Feb 2021
  left_join(amp2yr, by = c("ptNum", "ptSide")) %>%
  left_join(amp1yr, by = c("ptNum", "ptSide")) %>% # Added 11 Feb 2021
  left_join(mutate(grid, ptNum = as.character(ptNum)), by = c("ptNum", "ptSide")) %>%
  replace_na(list(majorAmp2yr = 0, 
                  majorAmp1yr = 0, # Added 11 Feb 2021
                  RunoffDisease = 0,
                  InflowDisease = 0,
                  OutflowDisease = 0)) %>%
...
analyticData <- preAnalyticData2 %>%
  ...
  select(ptNum:CAD, anyAnemia, anyHyperlipidemia, mort2yr, majorAmp2yr, mace2yr, mort1yr, majorAmp1yr) %>%
  mutate(ampFreeSurv2yr = if_else(mort2yr == 0 & majorAmp2yr == 0 , 1, 0)) %>%
  filter(!is.na(CAD)) 


# Combining CHF and Diabeters per e-mail from Kate to Nikki on 5 April 2021 + 
# Combining categories to match the C-TMLE analysis
analyticData <- analyticData %>%
  # CHF and complicated CHF combined into anyCHF
  mutate(anyCHF = if_else(CHF == 1 | `CONGESTIVE HEART FAILURE-COMPLICATED` == 1, 1, 0)) %>%
  # Diabetes and uncomplicated diabetes combined into diabetes; any diabetes
  mutate(diabetes = if_else(DIABETES == 1 | `DIABETES-UNCOMPLICATED` == 1, 1, 0)) %>%
  mutate(anyDiabetes = if_else(`DIABETES-COMPLICATED` == 1 | diabetes == 1, 1, 0 )) %>%
  # hypertension
  mutate(anyHypertension = if_else(`HYPERTENSION-COMPLICATED` == 1 | `HYPERTENSION-UNCOMPLICATED` == 1, 1, 0)) %>%
  # renal
  mutate(anyRenal = if_else(`RENAL DISEASE-CKD` == 1 | `RENAL DISEASE-COMPLICATED` == 1 | `RENAL DISEASE-ESRD` == 1 | RENAL_DISEASE == 1, 1, 0))
  
# readr::write_csv(x = analyticData, file = "../../../Consulting Project/Data/analyticData.csv")
...
analyticData <- read_csv("/Users/nikkifreeman/Dropbox/Consulting Project/Data/analyticData.csv") %>%
  mutate(ptNum = as.character(ptNum),
         CLI_DX_DATE = ymd(CLI_DX_DATE))

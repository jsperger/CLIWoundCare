...
set.seed(42)
mort.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 500, 
...
set.seed(303)
amp.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 500, 
...
set.seed(24)
mace.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 500, 
...
aim2.plot <- ggplot(data = results.sum.df, aes(x = y, y = RD.hat,  ymin = Lower, ymax = Upper))+
  ...
  geom_text(aes(label = round(RD.hat, 2)),
            ...
            y = c(0.11, 0.05, 0.01, -0.02, -0.13, -0.1, 0.14, 0.13, 0.09, 0.05, 0.1, 0.05),
            size = 3) + 
